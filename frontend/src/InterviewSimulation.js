...
const InterviewSimulation = () => {
  ...
  // Keep a stable ref for isMuted so speak() always reads the latest value
  const isMutedRef = useRef(isMuted);
  useEffect(() => { isMutedRef.current = isMuted; }, [isMuted]);

  const bottomRef = useRef(null);

  // ── speak() — stable, reads isMuted from ref ──
  const speak = useCallback((text) => {
    if (!ttsSupported || isMutedRef.current) return;
    window.speechSynthesis.cancel();

    // Strip symbols that sound odd when spoken aloud
  ...
  const toggleMute = useCallback(() => {
    if (!isMutedRef.current) stopSpeaking();   // stop immediately on mute
  ...
  // ── Speak first message only AFTER popup is dismissed ──
  // Stable refs so popup-close effect satisfies exhaustive-deps without re-triggering
  const messagesRef = useRef(messages);
  useEffect(() => { messagesRef.current = messages; }, [messages]);
  const speakRef = useRef(speak);
  useEffect(() => { speakRef.current = speak; }, [speak]);

  // Speak first AI message only after popup is dismissed
  useEffect(() => {
    if (showPopup) return;
  ...
  const sendMessage = async () => {
    ...
    stopSpeaking();   // stop ongoing TTS when user sends reply
    ...
    try {
      ...
      const data = await res.json();
      if (data.reply) { setMessages([...updated, { role: "assistant", content: data.reply }]); speak(data.reply); }
  ...
  // ── Shared popup close handler — closes popup then speaks first message ──
  const closePopup = () => {
    setShowPopup(false);
    // speak() is triggered by the useEffect watching showPopup
  };
  ...
  const MuteButton = () => (
    <button onClick={toggleMute}
      ...
      style={{
        ...
        fontSize:11, fontWeight:700, transition:"all 0.2s ease", flexShrink:0,
        background: isMuted ? "#fee2e2" : "#f0fdf6",
        border: `1px solid ${isMuted ? "#fca5a5" : "#b6e8cc"}`,
        color: isMuted ? "#ef4444" : "#4caf82",
      }}
    >
  ...
  return (
    <div className="relative min-h-screen overflow-hidden font-display text-gray-800">
      <style>{`
        ...
        @keyframes glowPulse {
          0%,100%{box-shadow:0 20px 50px rgba(0,0,0,0.08),0 0 0 1px rgba(210,190,140,0.25),0 0 14px 2px rgba(220,205,160,0.10)}
          50%    {box-shadow:0 20px 50px rgba(0,0,0,0.08),0 0 0 1px rgba(220,200,150,0.42),0 0 20px 4px rgba(230,215,170,0.15)}
        }
        ...
        /* subtle green glow on the last AI bubble while speaking */
        @keyframes speakPulse {
          0%,100%{box-shadow:0 2px 10px rgba(100,120,220,0.08)}
          50%    {box-shadow:0 0 16px 4px rgba(76,175,130,0.22)}
        }
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen px-4">
        <div className="w-full max-w-6xl flex items-stretch">
          ...
          {/* ── ROBOT AVATAR SIDE PANEL — redesigned ── */}
          <div className="flex-shrink-0" style={{
            width: 220,
            minHeight: 480,
            borderRadius: "28px 0 0 28px",
            background: "linear-gradient(160deg, #f0f7ff 0%, #e8f4ea 40%, #f5eeff 100%)",
            border: "1.5px solid rgba(255,255,255,0.9)",
            borderRight: "none",
            boxShadow: isSpeaking
              ? "0 8px 40px rgba(91,141,238,0.22), 0 2px 12px rgba(163,177,138,0.15), inset 0 1px 0 rgba(255,255,255,0.9)"
              : "0 8px 32px rgba(163,177,138,0.14), 0 2px 8px rgba(0,0,0,0.06), inset 0 1px 0 rgba(255,255,255,0.9)",
            ...
            <div style={{width:"100%",textAlign:"center",zIndex:1}}>
              <div style={{
                ...
                textTransform:"uppercase",color:"#5b7a6a",
            ...
            <div style={{position:"relative",display:"flex",alignItems:"center",justifyContent:"center",width:180,height:200,zIndex:1}}>
              ...
              {/* Soft glow base circle */}
              <div style={{
                position:"absolute",width:140,height:140,borderRadius:"50%",
                background: isSpeaking
                  ? "radial-gradient(circle, rgba(91,141,238,0.12) 0%, rgba(163,177,138,0.08) 60%, transparent 100%)"
                  : "radial-gradient(circle, rgba(163,177,138,0.10) 0%, transparent 70%)",
                transition:"background 0.5s ease",
              }}/>

              {/* ── NEW POLISHED ROBOT SVG ── */}
              <svg viewBox="0 0 120 150" xmlns="http://www.w3.org/2000/svg"
                className={isSpeaking ? "" : "robot-float"}
                style={{width:160,height:190,filter:`drop-shadow(0 8px 24px rgba(91,130,180,0.22)) drop-shadow(0 2px 6px rgba(0,0,0,0.10))`}}>
                <defs>
                  ...
                  <linearGradient id="eyeGrad" x1="0%" y1="0%" x2="0%" y2="100%">
                    ...
                    <stop offset="100%" stopColor="#5b9ee8"/>
                  </linearGradient>
                  <linearGradient id="eyeGradG" x1="0%" y1="0%" x2="0%" y2="100%">
                  ...
                  <linearGradient id="armGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                    ...
                    <stop offset="100%" stopColor="#b8cade"/>
                  </linearGradient>
                  <filter id="robotGlow">
                    <feGaussianBlur stdDeviation="1.5" result="blur"/>
                    <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
                  </filter>
                  <radialGradient id="screenGrad" cx="50%" cy="30%" r="70%">
                    <stop offset="0%" stopColor="#f0f8ff" stopOpacity="0.95"/>
                    <stop offset="100%" stopColor="#ddeeff" stopOpacity="0.7"/>
                  </radialGradient>
                </defs>
                ...
                {/* Eye iris */}
                <ellipse cx="39" cy="42" rx="9" ry="9"
                  fill={isSpeaking ? "url(#eyeGradG)" : "url(#eyeGrad)"}
                  className="robot-eye"
                  style={{transition:"fill 0.4s ease"}}/>
                <ellipse cx="81" cy="42" rx="9" ry="9"
                  fill={isSpeaking ? "url(#eyeGradG)" : "url(#eyeGrad)"}
                ...
                {/* Glow ring around eyes when speaking */}
                {isSpeaking && (<>
                ...
                {isSpeaking ? (
                  // Speaking: bouncing equalizer bars
                  <>
                    {[0,1,2,3,4,5,6].map((i) => (
                      <rect key={i}
                        x={35 + i*7} y="61" width="4" height="7" rx="2"
                        fill={i%2===0 ? "#81D4FA" : "#80cbc4"}
                        style={{transformOrigin:`${37+i*7}px 64.5px`, animation:`mouthBounce 0.${2+i%3}s ease-in-out ${i*0.06}s infinite`}}/>
                    ))}
                  </>
                ) : (
                  // Idle: calm smile arc
                  <path d="M 38 64 Q 60 70 82 64" stroke="#81D4FA" strokeWidth="2.5" fill="none" strokeLinecap="round" opacity="0.8"/>
                )}
                ...
                {/* Ear detail */}
                <circle cx="13" cy="39" r="3" fill={isSpeaking ? "#4caf82" : "#90a4ae"} style={{transition:"fill 0.3s ease",boxShadow: isSpeaking ? "0 0 8px #4caf82" : "none"}} className={isSpeaking ? "antenna-dot" : ""}/>
                ...
                {/* Chest display — waveform when speaking, grid when idle */}
                {isSpeaking ? (
                  <path d="M 32 117 Q 37 107 42 117 Q 47 127 52 117 Q 57 107 62 117 Q 67 127 72 117 Q 77 107 82 117 Q 87 127 88 117"
                    stroke="#5b8dee" strokeWidth="2" fill="none" strokeLinecap="round"
                    style={{animation:"mouthBounce 0.4s ease-in-out infinite"}}/>
                ) : (<>
                  {[0,1,2,3].map(i=>(
                    <line key={i} x1="32" y1={107+i*6} x2="88" y2={107+i*6} stroke="#90a4ae" strokeWidth="0.8" opacity="0.3"/>
                  ))}
                  {[0,1,2,3,4,5,6].map(i=>(
                    <line key={i} x1={32+i*8} y1="107" x2={32+i*8} y2="131" stroke="#90a4ae" strokeWidth="0.8" opacity="0.3"/>
                  ))}
                </>)}

                {/* Status lights row */}
                {[0,1,2,3,4].map((i)=>(
                  <circle key={i} cx={36+i*12} cy="136" r="4"
                    fill={isSpeaking ? ["#A3B18A","#5b8dee","#ef9a9a","#80cbc4","#f9a825"][i] : "#b0bec5"}
                    style={isSpeaking ? {animation:`antennaPulse 0.5s ease-in-out ${i*0.1}s infinite`} : {transition:"fill 0.3s ease"}}/>
                ))}
            ...
            <div style={{textAlign:"center",zIndex:1,width:"100%"}}>
              <div style={{fontSize:14,fontWeight:900,color:"#2d4a6a",letterSpacing:"-0.01em",lineHeight:1}}>
                PREPBOT
              </div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:3,height:28,zIndex:1,width:"100%"}}>
              {[4,7,5,9,6,8,4,7,5].map((h,i)=>(
                <div key={i} style={{
                  ...
                  background: isSpeaking
                    ? `linear-gradient(to top, ${selectedPersona.color}, #5b8dee)`
                    : "linear-gradient(to top, #d1dde8, #b8cad8)",
                  height: isSpeaking ? `${h*3}px` : "6px",
                  transition: "height 0.15s ease, background 0.3s ease",
                  animation: isSpeaking ? `mouthBounce 0.${3+i%4}s ease-in-out ${i*0.05}s infinite` : "none",
            ...
            <svg viewBox="0 0 220 480" xmlns="http://www.w3.org/2000/svg"
              ...
              {/* Small sparkle dots */}
              {[[30,40],[180,70],[20,200],[195,160],[40,380],[175,340],[100,420],[160,430]].map(([x,y],i)=>(
                <circle key={i} cx={x} cy={y} r={i%3===0?2:1.2}
                  fill={["#5b8dee","#A3B18A","#ef9a9a","#80cbc4"][i%4]}
                  opacity={isSpeaking ? 0.5 : 0.2}
                  style={isSpeaking ? {animation:`antennaPulse ${0.8+i*0.15}s ease-in-out ${i*0.1}s infinite`} : {transition:"opacity 0.4s ease"}}/>
              ))}
              {/* Corner accent lines */}
            ...
            <div className="chat-scroll h-80 overflow-y-auto rounded-2xl p-4 mb-4 space-y-4"
              ...
              {messages.map((msg, idx) => {
                ...
                return (
                  <div key={idx}
                    className={
                      msg.role === "assistant"
                        ? `${isLastAI && isSpeaking ? "msg-speaking" : "msg-bubble"} max-w-[85%] whitespace-pre-line text-sm leading-relaxed mr-auto rounded-2xl rounded-tl-none px-4 py-3`
                        : "msg-bubble max-w-[85%] whitespace-pre-line text-sm leading-relaxed ml-auto rounded-2xl rounded-tr-none px-4 py-3 text-right"
                    }
                    style={msg.role === "assistant"
                      ? { background:"linear-gradient(135deg,#eef3ff,#e8f0ff)", color:"#2a3a8a", border:"1px solid #d4dcf8" }
                      : { background:"linear-gradient(135deg,#e8f4ea,#dff0e2)", color:"#1a5a2a", border:"1px solid #b8ddc4", boxShadow:"0 2px 10px rgba(60,140,80,0.08)" }}
                  >
              ...
              {loading && (
                <div className="mr-auto rounded-2xl rounded-tl-none px-4 py-3 flex items-center gap-1"
                  style={{ background:"linear-gradient(135deg,#eef3ff,#e8f0ff)", border:"1px solid #d4dcf8", width:"fit-content" }}>
            ...
            <div className="flex items-center gap-3">
              ...
              {voiceSupported && (
                <button onClick={toggleVoice} disabled={loading}
                  ...
                  className={`voice-btn w-12 h-12 rounded-xl flex items-center justify-center border-2 ${isListening ? "listening bg-red-500 border-red-500 text-white" : "bg-white border-gray-300 text-gray-600 hover:border-[#B7C7A1] hover:text-[#5a7a4a]"}`}>
          ...
          </div>{/* end chat card flex-1 wrapper */}
        </div>{/* end max-w-6xl flex row */}
      </div>

      {/* Welcome Popup — untouched */}
      {showPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="popup-card relative w-full max-w-md rounded-2xl bg-gradient-to-br from-[#eef3ff] via-[#f0f7f3] to-[#e8f4ea] border border-[#cfd4df] shadow-[0_25px_60px_rgba(0,0,0,0.2)] p-6 pt-10">
            <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-20 h-20 rounded-full bg-white shadow-md flex items-center justify-center border border-[#cfd4df]"
            ...
            <ul className="mb-4" style={{ padding:0 }}>
              ...
              ].map((item, i) => <li key={i} className="popup-li">{item.icon} {item.text}</li>)}
            </ul>
            <div className="text-right">
              <button onClick={closePopup} className="send-btn px-4 py-2 rounded-lg bg-[#A3B18A] text-white font-medium hover:bg-[#8F9D78] transition">
                Start Interview
              </button>
            </div>
          </div>
